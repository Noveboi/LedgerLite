    ...
    public DateRange? GetOverlapWith(DateRange other)
    {
        ...
        if (Start > End || other.Start > other.End)
            throw new InvalidOperationException(
                message: "Start date must be less than or equal to End date in both ranges.");
        ...
        if (overlapStart <= overlapEnd) return new DateRange(Start: overlapStart, End: overlapEnd);
